...
type RedisAdapter interface {
	...
	GetCommentsByArticle(articleID int) data.CommentEntities
	...
	GetLikeCommentsByComment(commentID int) data.LikeCommentEntities
}
...
func (a *RedisAdapterImpl) RefreshLikeCommentsByUserAndComment(username string, commentID int) {
	var (
		key = fmt.Sprintf("khumu-comment:likeComments:user=%s&comment=%d", username, commentID)
		val = data.LikeCommentEntities(make([]*ent.LikeComment, 0))
	)
	val, err := a.likeCommentRepository.FindAllByUserIDAndCommentID(username, commentID)
	if err != nil {
		log.Error(err)
		return
	}
...
func (a *RedisAdapterImpl) GetLikeCommentsByComment(commentID int) data.LikeCommentEntities {
	var (
		key = fmt.Sprintf("khumu-comment:likeComments:comment=%d", commentID)
		val = data.LikeCommentEntities(make([]*ent.LikeComment, 0))
	)
